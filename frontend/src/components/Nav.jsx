...
const NavItem = ({ to, icon, label }) => (
  <NavLink
    to={to}
    className={({ isActive }) =>
      `flex flex-col items-center justify-center w-12 h-12 transition-colors duration-300 ${
        isActive ? 'text-primary-green' : 'text-white/40 hover:text-white/70'
      }`
    }
  >
    ...
    <nav className="fixed bottom-8 left-1/2 z-100 w-[90%] max-w-sm -translate-x-1/2">
      ...
      {showAskKaya && (
        <div className="absolute -top-14 left-1/2 -translate-x-1/2">
          <button
            onClick={() => console.log('Ask Kaya clicked')}
            aria-label="Ask Kaya"
      ...
      <div className="glass-panel flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 p-2 shadow-2xl backdrop-blur-md">
        ...
        <div className="relative mx-2">
          <NavLink
            ...
            className={({ isActive }) =>
              `glass-button-primary flex h-14 w-20 items-center justify-center rounded-full border border-primary-green/60 shadow-[0_0_20px_rgba(34,255,136,0.3)] transition-all duration-300 ${
                isActive 
